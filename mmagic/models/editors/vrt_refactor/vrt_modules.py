...
class VRTModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, 
        ...
        self.residual_group1 = TMSAG(n_channels=out_channels, n_blocks=n_mma_blocks, 
                                     depth=depth, n_heads=n_heads, window_size=window_size, 
        ...
        # parallel warping
        self.pa_deform = DCNv2PackFlowGuided(out_channels, out_channels, n_frames=n_frames, 
                                             deformable_groups=deformable_groups, kernel_size=kernel_size, 
                                             padding=padding, max_residue_magnitud=max_residue_magnitude)
...
class VRTMod8(nn.Module):

    def __init__(self, in_channels, out_channels, 
                 n_blocks,
                 depth, n_heads, window_size, 
                 mlp_ratio, qkv_bias, qk_scale, 
                 drop_paths, norm_layer,
                 indep_layers,
                 ):
        super(VRTMod8, self).__init__()
        # stage 8
        ...
        self.tfn = nn.ModuleList([
            RTMSA(n_channels=out_channels,
                depth=depth,
                n_heads=n_heads,
                window_size=[1, window_size[1], window_size[2]] if i in indep_layers else window_size, # T=1 or T=all
                mlp_ratio=mlp_ratio,
